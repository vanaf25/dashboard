...
const GeneratePdf:React.FC<GeneratePDFProps> = ({elems,data,
                                                    pdfTitle,withOutHeader}) => {
    return (
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                {elems.map((el, index) => {
                    ...
                    return (
                        <>
                        {el.type!==ElementType.SECTION ? <Text key={index} style={getStyle()}>
                            {el.content.replace(/\n\s+/g, " ")}
                        </Text>:<View  key={index}  style={styles.section}>
                            {el.title && <Text style={styles.heading}>{el.title}</Text>}
                            {<Text style={el.title ? styles.description:styles.text}>{el.content.replace(/\n\s+/g, " ")}</Text>}
                        </View>}
                        </>
                    );
                })}

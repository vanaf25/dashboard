...
const Expectations = () => {
  ...
  return (
      <WorkerLayout
          withOutHeader
          pdfTitle={"Expectations"}
          pdfName={"Expectations"}
          exportElems={
        [
            ...standardExpectations.map(el=>({ type: ElementType.SECTION, content: el })),
          {type:ElementType.H3,content:"Mandatory Daily Tasks:"},
            ...mandatoryTasks.map(el=>({type:ElementType.SECTION,content:el}))
        ]
      }
      >
        <Typography variant="h4" sx={{ mb: 1 }}>Standard Expectations</Typography>
          {standardExpectations.map((el, index) => (
              <BlankCard sx={{mb:1}} key={`standard-${index}`}>
                <Typography>{el}</Typography>
              </BlankCard>
          ))}

        <Typography variant="h4" sx={{ mt: 2, mb: 1 }}>Mandatory Daily Tasks</Typography>
          {mandatoryTasks.map((el, index) => (
              <BlankCard sx={{mb:1}} key={`task-${index}`}>
                <Typography>{el}</Typography>
              </BlankCard>
          ))}
      </WorkerLayout>

...
const DrugTestNotice = () => {
    const firstSection="Here after company refers to:Mister Exterior LLC"
  return (
    <WorkerLayout pdfName={"Drug Test Notice"} exportElems={
        [{type:ElementType.P,content:firstSection},
            ...content.map(el=>({type:ElementType.P,content:el}))]
    } pdfTitle={"Drug Test Notice"}>
    <Typography sx={{textAlign:"center",mb:2}} variant={"h3"}>
      Drug Test Consent
    </Typography>
    <Typography sx={{mb:1}}>{firstSection}</Typography>
     <Paper sx={{p:2}}>
         <Paragraphs sections={content} />
     </Paper>
    </WorkerLayout>
  );
};

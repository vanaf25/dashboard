...
const NonDisclosureAgreement = () => {
  ...
  // State for controlled TextField
  ...
  const headerText=` This Non Disclosure Agreement is
          entered into by and between 
          ${company?.companyName} with its principal offices at 
          ${receiveAddress(company)} AND ${data?.fullName} Located At `
  return (
      <WorkerLayout withSignature exportElems={[
        {type:ElementType.P,content:`${headerText},${workerAddress}`},
          ...sections.map(section=>({type:ElementType.SECTION,title:section.title,content:section.content}))
      ]}
                    pdfTitle={"Non Disclosure Agreement"}
                    pdfName={"Non Disclosure Agreement"}
      >

...
import RenovationRequestForm from "@/app/components/letters/RenovationRequestForm/RenovationRequestForm";
...
const NewClientQuestions = () => {
  ...
  const [data,setData]=useState<{field:string,value:string}[]>([])
  return (
    <WorkerLayout withOutHeader
                  exportElems={[
                      {type:ElementType.ListCard,cardListRows:data},
                      ...customerProcessSteps.map(el=>({type:ElementType.P,content:el}))
                  ]
                  }
                  pdfName={"New Client Questions"} pdfTitle={"New Client Questions"} >
      <Typography variant={"h4"}>New Client Questions</Typography>
      <RenovationRequestForm onUpdate={(val)=>setData(val)}/>
